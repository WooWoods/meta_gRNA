...
def run_bowtie(input_fa, bowtie_idx):
    ...
    cmd = (f"bowtie -v 0 -a {bowtie_idx} --suppress 6,7 \
...
def clapse_by_density(guides, grna_counts, bowtie_idx, grna_retained):
    input_fa = 'input.fa'
    ...
    while j < len(positions_sorted):
        if positions_sorted[j] - positions_sorted[i] > 50:
            i = j
            j += 1
            grna_retained.add(grna_positions[positions_sorted[i]])
            try:
                grna_retained.add(grna_positions[positions_sorted[j]])
            except IndexError:
                continue
            continue
        else:
            clusters = []
            clusters.append(i)
            while j < len(positions_sorted) and positions_sorted[j] - positions_sorted[i] <= 50:
                clusters.append(j)
                j += 1
            discarded = discard_for_dense(positions_sorted, clusters, grna_positions, grna_counts, grna_retained)
            discard_pos.extend(discarded)
            i = j
            j += 1
...
def discard_for_dense(positions_sorted, clusters, grna_positions, grna_counts, grna_retained):
    positions = [positions_sorted[i] for i in clusters]
    grnas = [grna_positions[p] for p in positions]
    counts = [grna_counts[grna] for grna in grnas]
    max_c = max(counts)
    max_idx = counts.index(max_c)
    
    retained = positions[max_idx]
    grna_retained.add(grnas[max_idx])
    positions.remove(retained)
    return positions
...
def load_filter_db(f):
    ...
    for i in range(cols):
        ...
        #    continue
        column = df.iloc[:, i]
        mean = column.quantile(.25)
        wanted = column[column > mean]
        data.append(wanted.to_dict())
    overlaps = set(data[0].keys()) & set(data[1].keys()) & set(data[2].keys()) & set(data[3].keys())
    print(len(overlaps))
    filter_db = defaultdict(int)
    for _id in overlaps:
        for df in data:
            filter_db[_id] += df[_id]
    return filter_db

def load_filter_db2(f):
    filter_db = defaultdict(int)
    with open(f) as fh:
        fh.readline()
        for line in fh:
            arr = line.strip().split()
            vals = list(map(lambda i:int(i), arr[1:]))
            if vals.count(0) > 2:
                continue
            filter_db[arr[0]] = max(vals)
    print(len(filter_db))
    return filter_db

def output_final(guides, grna_retained):
    drop = set()
    with open('filterout.txt') as fh:
        for line in fh:
            drop.add(line.strip())
    with open('guides_final.txt', 'w') as fo:
        for _id, seq in guides.items():
            if _id in grna_retained:
                fo.write(f'>{_id}\n{seq}\n')
...
def main(args):
    if args.command == 'meta':
        ...
        filter_db = load_filter_db(fgrna_amount)
        guides = read_guides(grnas)
        for _id in list(guides.keys()):
            if _id not in filter_db:
                del guides[_id]
        ...
        for path in os.listdir(genome_dir):
            ...
            bowtie_out = run_bowtie(input_fa, bowtie_idx)
            clapse_by_density(guides, filter_db, bowtie_idx, grna_retained)

            genome_fa = ''
            gtf = ''

        output_final(guides, grna_retained)
    ...
    if args.command == 'offtarget':
        ...
        with open('guides_rmoff.fa', 'w') as fh:
            for _id, seq in guides.items():
                fh.write(f'>{_id}\n{seq}\n')
...
if __name__ == '__main__':
    ...
    meta_parser.add_argument('--grna_amount', '-ga', default=False, type=str,
                        help='sgRNAs in fasta format')
